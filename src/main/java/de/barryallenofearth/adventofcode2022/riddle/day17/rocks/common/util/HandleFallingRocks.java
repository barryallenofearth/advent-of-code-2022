...
public class HandleFallingRocks {
	...
	public static final int STEPS_TO_DISPLAY = 1_000;
	...
	public static void handleFallingRocks(Cave cave, long numberOfRocks) {
		...
		for (long rockCount = 0; rockCount < numberOfRocks; rockCount++) {
			if (rockCount % STEPS_TO_DISPLAY == 0) {
				System.out.println(Duration.between(startingTime, LocalDateTime.now()).getSeconds() + "s " + rockCount / STEPS_TO_DISPLAY + "x10^3: " + ((double) rockCount) / numberOfRocks * 100. + "% processed");
			}
			lastMovementIndex = RockMover.handleNewRock(rockCount, lastMovementIndex, cave, gasStreams);
			if (rockCount >= Cave.NUMBER_OF_LINES) {
				...
				if (cycleValue.isPresent()) {
					...
					final long remainingCycles = (long) (((double) remainingRockCount) / cycleLength);
